...
const TUT_STEPS = [
  { emoji: "👋", title: "Benvenuto in Risparmia!", desc: "La tua app personale per tenere traccia di spese ed entrate mese per mese. In pochi secondi ti spieghiamo come funziona." },
  { emoji: "📅", title: "Il Calendario", desc: "Ogni giorno puoi registrare le tue spese. Usa i preset rapidi per le spese frequenti, o aggiungi una spesa personalizzata con categoria e nota. I pallini colorati indicano spese (verde), entrate (giallo) e spese fisse (viola)." },
  { emoji: "📌", title: "Spese fisse e periodiche", desc: "In fondo al calendario aggiungi spese mensili fisse (es. Netflix) con il giorno esatto, oppure spese periodiche (es. medicine del cane) che si ripetono ogni X giorni in un certo range del mese." },
  { emoji: "↕", title: "Le Entrate", desc: "Nella sezione Entrate registri stipendi, bonus e qualsiasi cosa entri. Puoi inserire la data esatta e l'entrata verra conteggiata nel mese giusto." },
  { emoji: "📊", title: "Statistiche e Storico", desc: "Le Statistiche ti mostrano spese per categoria, top 5 voci e proiezione del mese. Lo Storico ti permette di consultare qualsiasi mese passato con tutti i dettagli." },
  { emoji: "⚙", title: "Impostazioni", desc: "Dall'ingranaggio puoi cambiare tema, fare backup e ripristinare i dati. Il + in alto gestisce preset e categorie personalizzate. Buon risparmio!" },
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (d) {
      ...
      if (d.percMode) setPercMode(d.percMode);
    } else {
  ...
  useEffect(() => {
    saveData({ spese, entrate, presets, speseFisse, extraCats, themeId, percMode });
  }, [spese, entrate, presets, speseFisse, extraCats, themeId, percMode]);
  ...
  const statKey = `${statYear}-${statMonth}`;
  ...
  function calcSaldo(year, month, depth) {
    ...
    if (pm < 0) { pm = 11; py--; }
    const pk = `${py}-${pm}`;
    if (!entrate[pk] && !spese[pk]) return 0;
    ...
    const prevPer = speseFisse.filter(x => x.tipo === "periodica").reduce((acc, sp) => {
      const ini = sp.giornoInizio || 1, fin = sp.giornoFine || 28, fr = sp.freq || 1;
      let tot = 0;
      for (let g = ini; g <= fin; g += fr) tot += sp.importo;
      const last = ini + Math.floor((fin - ini) / fr) * fr;
      if (last < fin) tot += sp.importo;
      return acc + tot;
    }, 0);
    return prevSaldo + prevEnt - prevSp - prevFisse - prevPer;
  }

  const saldoPortato = useMemo(() => calcSaldo(curYear, curMonth, 0), [curYear, curMonth, spese, entrate, speseFisse]);

  const speseDelMese = useMemo(() => Object.values(spese[monthKey] || {}).flat(), [spese, monthKey]);

  function buildPeriodiche(fisse) {
    return fisse.filter(x => x.tipo === "periodica").reduce((acc, sp) => {
      const ini = sp.giornoInizio || 1, fin = sp.giornoFine || 28, fr = sp.freq || 1;
      for (let g = ini; g <= fin; g += fr) acc.push({ ...sp, giornoEffettivo: Math.round(g) });
      const last = ini + Math.floor((fin - ini) / fr) * fr;
      if (last < fin) acc.push({ ...sp, giornoEffettivo: fin });
      return acc;
    }, []);
  }
  ...
  const statTotaleSpese = statSpeseDelMese.reduce((s, x) => s + x.importo, 0)
    + speseFisse.filter(x => x.tipo !== "periodica").reduce((s, x) => s + x.importo, 0)
    + statPeriodiche.reduce((s, x) => s + x.importo, 0);
  const statTotaleEntrate = (entrate[statKey] || []).reduce((s, x) => s + x.importo, 0);
  const statDays = new Date(statYear, statMonth + 1, 0).getDate();
  const statTodayDay = today.getMonth() === statMonth && today.getFullYear() === statYear ? today.getDate() : statDays;
  const statMedia = statDays > 0 ? statTotaleSpese / statDays : 0;
  const statProiezione = statTodayDay > 0 ? (statTotaleSpese / statTodayDay) * statDays : 0;

  const statSpesePCat = useMemo(() => {
    const acc = {};
    statSpeseDelMese.forEach(x => { acc[x.cat] = (acc[x.cat] || 0) + x.importo; });
    speseFisse.filter(x => x.tipo !== "periodica").forEach(x => { acc[x.cat] = (acc[x.cat] || 0) + x.importo; });
    statPeriodiche.forEach(x => { acc[x.cat] = (acc[x.cat] || 0) + x.importo; });
    return Object.entries(acc).sort((a, b) => b[1] - a[1]);
  }, [statSpeseDelMese, speseFisse, statPeriodiche]);

  const statTop5 = useMemo(() => {
    const acc = {};
    statSpeseDelMese.forEach(x => { acc[x.desc] = (acc[x.desc] || 0) + x.importo; });
    speseFisse.filter(x => x.tipo !== "periodica").forEach(x => { acc[x.desc] = (acc[x.desc] || 0) + x.importo; });
    statPeriodiche.forEach(x => { acc[x.desc] = (acc[x.desc] || 0) + x.importo; });
  ...
  function addEntrata() {
    ...
    const d = new Date(entrataData);
    const eKey = `${d.getFullYear()}-${d.getMonth()}`;
    setEntrate(prev => {
      const list = [...(prev[eKey] || [])];
      list.push({ id: Date.now(), desc: entrataDesc, importo: amt, data: entrataData });
      return { ...prev, [eKey]: list };
    });
    setEntrataAmt("");
  }

  function removeEntrata(id, key) {
    setEntrate(prev => ({ ...prev, [key]: (prev[key] || []).filter(x => x.id !== id) }));
  }
  ...
  function addSpesaFissa() {
    ...
    setSpeseFisse(prev => [...prev, { id: Date.now(), desc: sfDesc, importo: amt, cat: sfCat, giorno: sfGiorno ? parseInt(sfGiorno) : null, tipo: "mensile" }]);
    setSfDesc(""); setSfAmt(""); setSfCat("altro"); setSfGiorno("");
  }

  function addSpesaPeriodica() {
    const amt = parseFloat(spAmt);
    const ini = parseInt(spGiornoInizio), fin = parseInt(spGiornoFine), fr = parseInt(spFreq) || 1;
    if (!spDesc.trim() || !amt || amt <= 0 || !ini || !fin || ini > fin) return;
    setSpeseFisse(prev => [...prev, { id: Date.now(), desc: spDesc, importo: amt, cat: spCat, giornoInizio: ini, giornoFine: fin, freq: fr, tipo: "periodica" }]);
    setSpDesc(""); setSpAmt(""); setSpCat("altro"); setSpGiornoInizio(""); setSpGiornoFine(""); setSpFreq("1");
  }

  function addExtraCat() {
    if (!newCatLabel.trim()) return;
    setExtraCats(prev => [...prev, { id: "cat_" + Date.now(), label: newCatLabel.trim(), emoji: newCatEmoji.trim() || "📌" }]);
    setNewCatLabel(""); setNewCatEmoji("");
  }

  function resetAll() {
    setSpese({}); setEntrate({});
    setPresets([{ id: 1, nome: "Sigarette", importo: 5.80, categoria: "vizi" }, { id: 2, nome: "Terea", importo: 5.50, categoria: "vizi" }]);
    setSpeseFisse([]); setExtraCats([]);
    setShowResetConfirm(false);
  }

  function handleGear() {
    if (tab === "impostazioni") { setTab(prevTab); }
    else { setPrevTab(tab); setTab("impostazioni"); }
  }
  ...
  const s = {
    ...
    iconBtn: { background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 8, color: "#e2e8f0", width: 36, height: 36, fontSize: 18, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" },
    ...
    seg: (active) => ({ flex: 1, padding: "7px 0", borderRadius: 8, border: "none", background: active ? theme.accent : theme.bg, color: active ? theme.bg : theme.sub, fontSize: 12, cursor: "pointer", fontWeight: active ? 600 : 400 }),
  };
  ...
  const KpiSection = () => (
    <div style={s.kpiGrid}>
      ...
      <div style={{ ...s.kpi, cursor: "pointer" }} onClick={() => setPercMode(m => m === "mese" ? "tot" : "mese")}>
        <p style={{ ...s.kpiLabel, color: percMode === "mese" ? theme.accent : theme.sub }}>{percMode === "mese" ? "% spesa/entrate mese" : "% spesa/entrate tot"}</p>
        <p style={s.kpiVal(percMode === "mese" ? theme.accent : "#94a3b8")}>{percSpesa !== null ? percSpesa + "%" : "---"}</p>
      </div>
    </div>
  );
    ...
    <div style={s.app}>
      <div style={{ padding: "16px 16px 8px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button style={s.iconBtn} onClick={() => setShowAddModal(true)}>+</button>
          <button onClick={handleGear} style={{ ...s.iconBtn, background: tab === "impostazioni" ? theme.accent : theme.card, color: tab === "impostazioni" ? theme.bg : "#e2e8f0" }}>⚙</button>
        </div>
      </div>
      ...
      {tab !== "impostazioni" && <div style={{ padding: "0 16px 12px" }}><KpiSection /></div>}
